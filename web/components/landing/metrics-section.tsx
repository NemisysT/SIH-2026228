...
/**
 * The reference's real-time metrics section, carrying the evidence summary.
 *
 * Preserved exactly: the wave-grid canvas behind the whole section, the
 * scrambling count-up that eases over 2.5s and blurs while it runs, the dotted
 * sparkline canvases with their per-card frequencies, the full-bleed organic
 * graph image, the 1+2 card grid, and the bottom mono ticker.
 *
 * The LIVE pill has become the source pill, and it says which body of data is
 * on screen. That distinction is load-bearing: the reference's badge asserted
 * freshness, this one asserts provenance, and an analyst who mistakes an
 * attack-lab scenario for a live assessment has been misled by the tool.
 *
 * The counts are the ones the engine already computed. Nothing here divides
 * one by another, and nothing here produces a percentage.
 */
...
export function MetricsSection({
  ...
  return (
    <section id="evidence" ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      <GridBackground />
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-20 lg:mb-32">
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6 flex-wrap">
              <span className={`flex items-center gap-2 px-3 py-1 text-xs font-mono uppercase tracking-widest ${
                live ? "bg-[#67e8f9]/10 text-[#67e8f9]" : "bg-[#eca8d6]/10 text-[#eca8d6]"
              }`}>
                <span aria-hidden="true" className={`w-2 h-2 rounded-full ${live ? "bg-[#67e8f9] animate-pulse" : "bg-[#eca8d6]"}`} />
                {live ? "LIVE ASSESSMENT" : "DEMO SCENARIO"}
              </span>
              <span className="text-sm font-mono text-muted-foreground">{generatedAt}</span>
            </div>
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[140px] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
              ...
              Evidence,
              <br />
              <span className="text-muted-foreground">counted once.</span>
            </h2>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          {/* Metrics */}
          {metrics.slice(1, 3).map((metric, index) => (
            <div
              key={metric.label}
        ...
        {/* Note + families ticker */}
        <p className={`mt-10 text-sm text-muted-foreground leading-relaxed max-w-3xl transition-all duration-1000 delay-400 ${
          isVisible ? "opacity-100" : "opacity-0"
        }`}>
          {note}
        </p>
        ...
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          ...
          <span className="text-foreground">Independent phenomena</span>
          {families.length === 0 ? (
            <span>none — no family carries unconfounded supporting evidence</span>
          ) : (
